...
int main(void)
{
	...
	S800_GPIO_Init();//Set PF0(LED) as Output pin and Set the PJ0,PJ1(two BUTTONs) as input pin
	S800_I2C0_Init();
	while (1)
	{
		...
		result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,0x0);  //turn on the LED1-8
...
uint8_t I2C0_WriteByte(uint8_t DevAddr, uint8_t RegAddr, uint8_t WriteData)
{
	...
	I2CMasterSlaveAddrSet(I2C0_BASE, DevAddr, false);	//set slave address, false means write, true means read
	I2CMasterDataPut(I2C0_BASE, RegAddr);                     //write reg address
	I2CMasterControl(I2C0_BASE, I2C_MASTER_CMD_BURST_SEND_START);//执行重复写入操作

	while(I2CMasterBusy(I2C0_BASE)){};
	rop = (uint8_t)I2CMasterErr(I2C0_BASE);                //detect error
	I2CMasterDataPut(I2C0_BASE, WriteData);
	I2CMasterControl(I2C0_BASE, I2C_MASTER_CMD_BURST_SEND_FINISH);//执行重复写入操作并结束

	while(I2CMasterBusy(I2C0_BASE)){};
	rop = (uint8_t)I2CMasterErr(I2C0_BASE);                //detect error
	return rop;                                                                 //return error type, return 0 if no error
}
...
uint8_t I2C0_ReadByte(uint8_t DevAddr, uint8_t RegAddr)
{
    ...
    // the first period(set slave address and write mode)(which is ready to write command to the reg)
	while(I2CMasterBusy(I2C0_BASE)){};
	I2CMasterSlaveAddrSet(I2C0_BASE, DevAddr, false);   //set slave address, false means write, true means read
	I2CMasterDataPut(I2C0_BASE, RegAddr);   //	I2CMasterControl(I2C0_BASE, I2C_MASTER_CMD_BURST_SEND_START);
	I2CMasterControl(I2C0_BASE,I2C_MASTER_CMD_SINGLE_SEND);//执行单词写入操作

	// the second period(write to the input port)
	while(I2CMasterBusBusy(I2C0_BASE));
	rop = (uint8_t)I2CMasterErr(I2C0_BASE);
	Delay(1);	//receive data

	// the third period(set slave address and read mode)(which is ready to read from the reg)
	I2CMasterSlaveAddrSet(I2C0_BASE, DevAddr, true);      //set slave address, false means write, true means read

	// the fourth period(read data from the reg)
	I2CMasterControl(I2C0_BASE,I2C_MASTER_CMD_SINGLE_RECEIVE);//执行单次读操作
	while(I2CMasterBusBusy(I2C0_BASE));
	value=I2CMasterDataGet(I2C0_BASE);            //receiving data
    Delay(1);
	return value;
}
