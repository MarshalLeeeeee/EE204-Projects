...
volatile uint8_t rightshift2 = 0x81;
	...
	while (1)
	{
		...
		if (systick_100ms_status)
		{
			...
			if (++i2c_flash_cnt		>= I2C_FLASHTIME/100)
			{
				...
				result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0x00);
				result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[cnt+2]);
				if(rightshift == 0x80) result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,rightshift << 1);
				else result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0x01);
				Delay(20000);

				result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~rightshift2);

				if(rightshift2 == 0x81)  rightshift2 = 0x03;
				else rightshift2 = rightshift2 << 1;
                cnt++;
