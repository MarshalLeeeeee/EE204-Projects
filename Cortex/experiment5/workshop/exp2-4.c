...
int main(void)
{
   ...
   //ui32SysClock = SysCtlClockFreqSet((SYSCTL_XTAL_25MHZ |SYSCTL_OSC_MAIN | SYSCTL_USE_PLL |SYSCTL_CFG_VCO_480), 120000000);;
    ...
    SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);  // divide frequency by 1000?
	SysTickEnable();
	SysTickIntEnable();
    IntMasterEnable();
	...
	while (1)
	{
		if (systick_10ms_status) // when  systick_10ms_count == 0, systick_10ms_status = 1
		{
		...
		if (systick_100ms_status)
		{
			...
			if (++i2c_flash_cnt		>= I2C_FLASHTIME/100)
			{
				...
				if (GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0)) // when is released
				{
					cnt++;
...
void SysTick_Handler(void)
{
	...
	{
		systick_100ms_couter = SYSTICK_FREQUENCY/10; //value = 100
	...
	{
		systick_10ms_couter	= SYSTICK_FREQUENCY/100; // value = 10
