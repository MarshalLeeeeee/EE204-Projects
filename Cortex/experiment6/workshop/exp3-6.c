...
void UART0_Handler(void)
{
	...
	tmp[index] = 0;
	if(judeg_format(tmp, SET))
	{

	}
	else if(judge_format(tmp, INC))
    {

    }
    else if(!strcmp(tmp, "GETTIME"))
    {
...
bool judge_format(const char *str, const char *order)
{
    if(!strncmp(str, order, 3) && str[5] == str[8] == ':'
       && isdigit(str[3]) && isdigit(str[4]) && isdigit(str[6])
       && isdigit(str[7]) && isdigit(str[9]) && isdigit(str[10])) return 1;
    else return 0;
}
