...
void UART0_Handler(void)
{
  ...
  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
		...
		tmp[index++] = toupper(UARTCharGetNonBlocking(UART0_BASE));
		GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,GPIO_PIN_1 );    // turn on the LED
        //Delay(1000);
	}
	tmp[index] = 0;
	//UARTStringPut((uint8_t *)tmp);
	if(!strcmp((char *)tmp, "AT+CLASS")) UARTStringPutNonBlocking(CLASS);
	if(!strcmp((char *)tmp, "AT+STUDE")) {id_flag = 1; GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,0 ); return ;}
	if(id_flag)
	{
		//UARTStringPut((uint8_t *)tmp);
		if(!strcmp((char *)tmp, "NTCODE")) UARTStringPutNonBlocking(ID);
		id_flag = 0;
	}
