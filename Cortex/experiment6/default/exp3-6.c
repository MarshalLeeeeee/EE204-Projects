...
#define	I2C_FLASHTIME				1000				//500mS
...
bool judge_format(char const *str, const char *order);
...
volatile uint8_t tmp[20];
volatile uint8_t id_flag = 0x0;
uint32_t ui32SysClock;
uint8_t seg7[] = {0x3f,0x06,0x5b,0x4f,0x66,0x6d,0x7d,0x07,0x7f,0x6f,0x77,0x7c,0x58,0x5e,0x079,0x71,0x5c};
uint8_t uart_receive_char;
volatile uint8_t hour = 0, sec = 0, min = 0, cnt_en = 0x1;
const char *CLASS = "CLASS15";
const char *ID = "CODE515030910361";
const char *SET = "SET";
const char *INC = "INC";
int main(void)
{
   ...
   //ui32SysClock = SysCtlClockFreqSet((SYSCTL_XTAL_25MHZ |SYSCTL_OSC_MAIN | SYSCTL_USE_PLL |SYSCTL_CFG_VCO_480), 120000000);;
	...
	while (1)
	{
		...
		if (systick_100ms_status)
		{
			...
			if (++i2c_flash_cnt >= I2C_FLASHTIME/100 && cnt_en)
			{
				i2c_flash_cnt	= 0;
				if (sec++ >= 59)
				{
		...
		result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[hour / 10]);
...
void UART0_Handler(void)
{
  ...
  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
		...
		tmp[index++] = UARTCharGetNonBlocking(UART0_BASE);
		GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,GPIO_PIN_1 );    // turn on the LED
        //Delay(1000);
	}
	tmp[index] = 0;
	//UARTStringPut((uint8_t *)tmp);
	if(!strncmp((char *)tmp, "SET", 3))
	{
		id_flag = 1;
		hour = (tmp[3] - '0') * 10 + tmp[4] - '0';
		min  = (tmp[6] - '0') * 10 + tmp[7] - '0';
		GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,0);
		return ;
	}
	if(!strncmp((char *)tmp, "INC", 3))
	{
		id_flag = 2;
		hour += (tmp[3] - '0') * 10 + tmp[4] - '0';
		min  += (tmp[6] - '0') * 10 + tmp[7] - '0';
	...
	GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,0 );                          // turn off the LED when interrupt is over
}
